...
export const EditCategoriesModal = ({ editCategoryModalOpen, setEditCategoryModalOpen, showSnackbar, categories, setCategories, ads, setAds }) => {
    ...
    const handleDeleteCategory = async (id) => {
        try {
            ...
            if (response.status == 200) {
                ...
                setCategories(updatedCategories);
                const updatedAds = ads.filter(book => book.category.id !== id);
                setAds(updatedAds);
            }
            else {
                showSnackbar('Server error', { variant: 'error' });
